...
namespace lunk {
	...
	SDLRenderer::SDLRenderer(void *nativeRenderer) {
		impl = new Implementation{(SDL_Renderer *) (nativeRenderer)};
	...
	void SDLRenderer::drawTexture(const Texture *source, const Vec2 &position, const Rect2 *sourceRect,
		...
		if (sourceRect) {
			srcRect = {(sourceRect->x), (sourceRect->y), (sourceRect->w), (sourceRect->h)};
			hasSourceRect = true;
		}

		if (sourceRect) {
			dstRect = {(int) position.x, (int) position.y, (sourceRect->w), (sourceRect->h)};
		} else {
		...
		SDL_RenderCopyEx(impl->renderer, reinterpret_cast<SDL_Texture *>(source->getNative()),
						 hasSourceRect ? &srcRect : (SDL_Rect *) nullptr, &dstRect, rotation, nullptr, SDL_FLIP_NONE);
	}
	...
	Texture *SDLRenderer::createTexture(const Vec2 &size, int access = SDL_TEXTUREACCESS_TARGET) {

		SDL_Texture *sdlTexture = SDL_CreateTexture(impl->renderer, SDL_PIXELFORMAT_RGBA8888, access, (int) size.w,
													(int) size.h);
	...
	void SDLRenderer::setAlphaModulation(Texture *texture, int modulation) {
		int error = SDL_SetTextureAlphaMod(reinterpret_cast<SDL_Texture *>(texture->getNative()),
										   (Uint8) (modulation & 0xFF));
		if (error) {
	...
	Vec2 SDLRenderer::getViewportSize() {
		...
		return lunk::Vec2{(double) viewportRect.w, (double) viewportRect.h};
	}
